<template>
    <div class="demo-input-suffix">
        ...
        <el-form label-position="right" :rules="formRule" :model="formModel" ref="formRef" label-width="150px">
            <el-form-item>
                <span style="color: #e64242">前提：手机号已签约B公司，且发任务走反向导入，须已设置成反向任务自动领取</span>
            </el-form-item>
            <el-form-item label="任务类型" prop="acceptanceType">
                <!--                所有的元素都包在校验规则模块中，因此动态绑定的属性名要用formModel.publicTaskType格式，而不是直接写publicTaskType，即该元素适用校验规则-->
                <el-radio-group v-model="formModel.acceptanceType">
                    <el-radio :label="'lgAcceptance'">临时个体</el-radio>
                    <el-radio disabled :label="'fqAcceptance'">非全</el-radio>
                    <el-radio disabled :label="'clAcceptance'">承揽</el-radio>
                </el-radio-group>
            </el-form-item>
            ...
            <el-form-item>
                <el-button type="primary"  @click="sub">生成B端待验收订单</el-button>
...
<script>
    ...
    import { queryUserName,createAcceptanceOrder } from '@/utils/publishTask'
    export default {
        name: 'acceptanceOrder',    //该vue文件的别名，其他文件请求该vue文件时可以直接使用别名
        data(){
            return {
                ...
                formModel: {
                    ...
                    tasknumber: '',
                    mobile: '',
                    acceptanceType: 'lgAcceptance',

                },
                // 校验规则，直接引用prop属性的值
                formRule: {
                    acceptanceType: [{ required: true, message: "请选择结算任务类型"}],
            ...
            sub(){
                ...
                this.$refs['formRef'].validate((valid) => {      //触发校验规则
                        if(valid){      //表示整个校验规则通过
                            ...
                            createAcceptanceOrder(this.formModel).then(res => {        //res是后台接口返回的数据
                                console.log(res)
                                alert('发布成功!')
                            })
                        }else{
