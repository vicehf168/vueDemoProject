<template>
    <div class="demo-input-suffix">
        ...
        <el-form label-position="right" :rules="formRule" :model="formModel" ref="formRef" label-width="150px">
            ...
            <el-form-item label="任务类型" prop="settlementType">
                <!--                所有的元素都包在校验规则模块中，因此动态绑定的属性名要用formModel.publicTaskType格式，而不是直接写publicTaskType，即该元素适用校验规则-->
                <el-radio-group v-model="formModel.settlementType">
                    <el-radio :label="'lgSettlement'">临时个体</el-radio>
                    <el-radio disabled :label="'fqSettlement'">非全</el-radio>
                    <el-radio disabled :label="'clSettlement'">承揽</el-radio>
                </el-radio-group>
            </el-form-item>
            ...
            <el-form-item>
                <el-button type="primary"  @click="sub">生成B端待结算订单</el-button>
...
<script>
    ...
    import { queryUserName,createSettlementOrder } from '@/utils/publishTask'
    export default {
        name: 'settlementOrder',    //该vue文件的别名，其他文件请求该vue文件时可以直接使用别名
        data(){
            return {
                ...
                formModel: {
                    ...
                    tasknumber: '',
                    mobile: '',
                    settlementType: 'lgSettlement',

                },
                // 校验规则，直接引用prop属性的值
                formRule: {
                    settlementType: [{ required: true, message: "请选择任务类型"}],
            ...
            sub(){
                ...
                this.$refs['formRef'].validate((valid) => {      //触发校验规则
                        if(valid){      //表示整个校验规则通过
                            ...
                            createSettlementOrder(this.formModel).then(res => {        //res是后台接口返回的数据
                                console.log(res)
                                alert('发布成功!')
                            })
                        }else{
