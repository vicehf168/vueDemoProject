<template>
    <div class="demo-input-suffix">
        ...
        <el-form label-position="right" :rules="formRule" :model="formModel" ref="formRef" label-width="150px">
            ...
            <el-form-item label="任务类型" prop="xvReviewType">
                <!--                所有的元素都包在校验规则模块中，因此动态绑定的属性名要用formModel.publicTaskType格式，而不是直接写publicTaskType，即该元素适用校验规则-->
                <el-radio-group v-model="formModel.xvReviewType">
            ...
            <el-form-item>
                <el-button type="primary"  @click="sub">生成小V端待审核订单</el-button>
...
<script>
    ...
    import { queryUserName,createXvReviewOrder } from '@/utils/publishTask'
    export default {
        name: 'xvReviewOrder',    //该vue文件的别名，其他文件请求该vue文件时可以直接使用别名
        data(){
            return {
                ...
                formModel: {
                    ...
                    tasknumber: '',
                    mobile: '',
                    xvReviewType: 'lgReview',

                },
                // 校验规则，直接引用prop属性的值
                formRule: {
                    xvReviewType: [{ required: true, message: "请选择任务类型"}],
            ...
            sub(){
                ...
                this.$refs['formRef'].validate((valid) => {      //触发校验规则
                        if(valid){      //表示整个校验规则通过
                            ...
                            createXvReviewOrder(this.formModel).then(res => {        //res是后台接口返回的数据
                                console.log(res)
                                if (res == 'success'){
                                    this.$message({
                                        message: '发布成功',
