<template>
    <div class="demo-input-suffix">
        ...
        <el-form label-position="right" :rules="formRule" :model="formModel" ref="formRef" label-width="150px">
            <el-form-item>
                <h4 style="color: #e64242">前提：1、手机号已签约B公司，且反向导入任务自动领取</h4>
                <h4 style="color: #e64242">2、开票内容已配置</h4>
                <h4 style="color: #e64242">3、走虚拟支付，需配置公司为虚拟支付方式及账号权限</h4>
                <h4 style="color: #e64242">4、收付款双方卡号已配置：contract_bank_info、transaction_bank_info（网商）</h4>
            </el-form-item>
            <el-form-item label="任务类型" prop="xvFeeIssueType">
                <!--                所有的元素都包在校验规则模块中，因此动态绑定的属性名要用formModel.publicTaskType格式，而不是直接写publicTaskType，即该元素适用校验规则-->
                <el-radio-group v-model="formModel.xvFeeIssueType">
                    <el-radio :label="'lgFeeIssueType'">临时个体</el-radio>
                    <el-radio disabled :label="'fqFeeIssueType'">非全</el-radio>
                    <el-radio disabled :label="'clFeeIssueType'">承揽</el-radio>
                </el-radio-group>
            </el-form-item>
            ...
            <el-form-item>
                <el-button type="primary"  @click="sub">生成小V端待费用发放订单</el-button>
...
<script>
    ...
    import { queryUserName,createXvFeeIssueOrder } from '@/utils/publishTask'
    export default {
        name: 'xvFeeIssueOrder',    //该vue文件的别名，其他文件请求该vue文件时可以直接使用别名
        data(){
            return {
                ...
                formModel: {
                    ...
                    tasknumber: '',
                    mobile: '',
                    xvFeeIssueType: 'lgFeeIssueType',

                },
                // 校验规则，直接引用prop属性的值
                formRule: {
                    xvFeeIssueType: [{ required: true, message: "请选择任务类型"}],
            ...
            sub(){
                ...
                this.$refs['formRef'].validate((valid) => {      //触发校验规则
                        if(valid){      //表示整个校验规则通过
                            ...
                            createXvFeeIssueOrder(this.formModel).then(res => {        //res是后台接口返回的数据
                                console.log(res)
                                alert('发布成功!')
                            })
                        }else{
