<template>
    <div class="demo-input-suffix">
        ...
        <el-form label-position="right" :rules="formRule" :model="formModel" ref="formRef" label-width="150px">
            ...
            <el-form-item label="任务类型" prop="payType">
                ...
                <el-radio-group v-model="formModel.payType">
                    <el-radio :label="'lgPay'">临时个体</el-radio>
                    <el-radio disabled :label="'fqPay'">非全</el-radio>
                    <el-radio disabled :label="'clPay'">承揽</el-radio>
                </el-radio-group>
            </el-form-item>
            ...
            <el-form-item>
                <el-button type="primary"  @click="sub">生成大V端待支付订单</el-button>
...
<script>
    ...
    import { queryUserName,createPayOrder } from '@/utils/publishTask'
    export default {
        name: 'payOrder',    //该vue文件的别名，其他文件请求该vue文件时可以直接使用别名
        data(){
            return {
                ...
                // 校验规则，直接引用prop属性的值
                formRule: {
                    payType: [{ required: true, message: "请选择任务类型"}],
            ...
            sub(){
                ...
                this.$refs['formRef'].validate((valid) => {      //触发校验规则
                        if(valid){      //表示整个校验规则通过
                            ...
                            createPayOrder(this.formModel).then(res => {        //res是后台接口返回的数据
                                console.log(res)
                                if (res == 'success'){
                                    this.$message({
                                        message: '发布成功',
